import React, { useState } from "react";

const Navbar = () => {
    const [navHandler, setNavHandler] = useState(false);

    const toogleNav = () => {
        setNavHandler(!navHandler);
    }
    return(
        <div className="  p-5  bg-light-violet md:flex  md:items-center md:justify-between">
            <div
                className="flex justify-between items-center"
            >
                <span
                    className="text-2xl"
                >
                    <img
                        className="h-10 inline"
                        src="data:image/png;base64,"
                    />
                        CreVa
                </span>
                <span
                className="w-12 h-10 ml-auto cursor-pointer md:hidden mx-2 block"
            >
                <img 
               src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b2/Hamburger_icon.svg/2048px-Hamburger_icon.svg.png"/>
            
            </span>
            </div>
            
            <ul className="md:flex md:items-center md:ml-auto z-[-1] md:z-auto md:static absolute w-full left:0 md:w-auto md:py-0 py-4 md:pl-0 pl-7 md:opacity-100 opacity-0">
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Inicio
                    </p>
                </li>
            
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Nuestros Servicios
                    </p>
                </li>

                    
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Equipo
                    </p>
                </li>
                    
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Blog
                    </p>
                </li>
            
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Contacto
                    </p>
                </li>
            
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0 ">
                        Iniciar Sesion
                    </p>
                </li>
            
                <li className="mx-4">
                    <p  className="text-xl hover:text-violet-500 duration-500 md:cursor-pointer my-6 md:my-0  bg-dark-violet p-2  mx-1 rounded-xl font-bold text-white">
                        Registrarse
                    </p>
                </li>
            
            </ul>
                        
        </div>
    )
}

export default Navbar;